...
class OUActionNoise(object):    # Ornstein-Uhlenbeck
    ...
    def __call__(self):
        # we can call an object like a function
        # noise = OUActionNoise()
        # noise()
...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, n_actions, name,
                 chkpt_dir='tmp/ddpg'):
        # beta is learning rate
        # chkpt_dir is check point directory
        ...
        f1 = 1 / np.sqrt(self.fc1.weight.data.size()[0])
            # a number for initializing the weights and biases of the layer of the neural network
        ...
        f3 = 0.003  # constant initialization
    ...
    def forward(self, state, action):
        ...
        state_value = F.relu(state_value)   # rectified linear unit function
...
class Agent(object):
    ...
    def choose_action(self, observation):
        self.actor.eval()
            # Making PyTorch to not calculate statistics for batch normalization
            # If we do not call eval() function, the agent will not learn
    ...
    def update_network_parameters(self, tau=None):
        # tau is a parameter that allows the update of the target network to 
        # gradually approach evaluation networks
        if tau is None:
            tau = self.tau
